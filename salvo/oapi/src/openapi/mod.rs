    ...
    fn merge_norm_node(&mut self, node: &mut NormNode, base_path: &str) {
        ...
        if let Some(handler_type_id) = &node.handler_type_id {
            if let Some(creator) = crate::EndpointRegistry::find(handler_type_id) {
                ...
                let _meta_not_exist_parameters = path_parameter_names
                    .iter()
                    .filter(|name| {
                        !name.starts_with('*')
                ...
                if !_meta_not_exist_parameters.is_empty() {
                    tracing::warn!(parameters = ?_meta_not_exist_parameters, path, handler_name = node.handler_type_name, "parameters information not provided");
                }
...
mod tests {
    ...
    fn test_build_openapi() {
        let _doc = OpenApi::new("pet api", "0.1.0")
            ...
            .add_security_scheme(
                "api_key",
                SecurityScheme::ApiKey(ApiKey::Header(ApiKeyValue::new("todo_apikey"))), // cspell: disable-line
            )
