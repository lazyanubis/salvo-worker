    ...
    S: SessionStore,
{
    ...
    pub fn build(self) -> Result<SessionHandler<S>, Error> {
        ...
        let fallback_hmac_list = fallback_keys
            .iter()
...
pub struct SessionHandler<S> {
    ...
    same_site_policy: SameSite,
    hmac: Hmac<Sha256>,
    fallback_hmac_list: Vec<Hmac<Sha256>>,
}
    ...
    S: SessionStore,
{
    ...
    fn verify_signature(&self, cookie_value: &str) -> Result<String, Error> {
        ...
        for hmac in &self.fallback_hmac_list {
    ...
    fn build_cookie(&self, secure: bool, cookie_value: String) -> Cookie<'static> {
        ...
        if let Some(ttl) = self.session_ttl {
            ...
            let date_time =
                time::OffsetDateTime::from_unix_timestamp((now / 1000.0) as i64 + ttl.as_secs() as i64).unwrap();
