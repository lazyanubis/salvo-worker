...
impl<'de> RequestDeserializer<'de> {
    ...
    pub(crate) fn new(request: &'de Request, metadata: &'de Metadata) -> Result<RequestDeserializer<'de>, ParseError> {
        ...
        if metadata.has_body_required()
            && let Some(c_type) = request.content_type()
        {
            match c_type.subtype() {
                mime::WWW_FORM_URLENCODED | mime::FORM_DATA => {
...
mod tests {
    ...
    async fn test_de_request_url_array() {
        ...
        let mut req = TestClient::get("http://127.0.0.1:5800/test/1234/param2v?ids=[3,2,11]").build();
        let data: RequestData = req.extract().await.unwrap();
        assert_eq!(
            data,
            RequestData {
        ...
        let mut req = TestClient::get(r#"http://127.0.0.1:5800/test/1234/param2v?ids=['3',  '2',"11","1,2"]"#).build();
        let data: RequestData = req.extract().await.unwrap();
        assert_eq!(
            data,
            RequestData {
                ids: vec!["3".to_string(), "2".to_string(), "11".to_string(), "1,2".to_string()]
            }
        );
    }
    ...
    async fn test_de_request_url_array2() {
        ...
        let mut req = TestClient::get("http://127.0.0.1:5800/test/1234/param2v?ids=[3,2,11]").build();
        let data: RequestData = req.extract().await.unwrap();
        assert_eq!(data, RequestData { ids: vec![3, 2, 11] });
    }
}
