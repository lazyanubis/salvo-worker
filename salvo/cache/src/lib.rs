    ...
    I: CacheIssuer,
{
    async fn handle(&self, req: &mut Request, depot: &mut Depot, res: &mut Response, ctrl: &mut FlowCtrl) {
        tracing::debug!("1 Cache middleware is handling request: {}", req.uri());
        if self.skipper.skipped(req, depot) {
            return;
        }
        tracing::debug!("2 Cache middleware is handling request: {}", req.uri());
        ...
        let cache = match self.store.load_entry(depot, &key).await {
            Some(cache) => cache,
            None => {
